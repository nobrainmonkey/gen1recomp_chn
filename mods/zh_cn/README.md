...
如果曾安装 0.1.0，请确认 MODS 页面显示 0.3.0 或更高版本。旧 0.1.0
会让大木博士开场保持英文，并可能在尼多力诺出现后崩溃。
...
4. Red、Blue、Yellow 的 NEW GAME 开场及游戏内冒烟测试。
5. ZIP 内容白名单检查，确保不存在 ROM、英文工作表、存档、缓存或个人路径。

## Gen1Recomp 更新维护

`manifest.json` 没有绑定单个 EXE 哈希，而是接受 `>=0.1.56 <1.0.0`。
大多数功能使用 Mod API 2；唯一明确的引擎内部兼容层是把选项页原始
`CANCEL` 页脚改为“取消”。更新引擎后应刷新字符串目录、查看
`src/ui/OptionRows.lua` 是否变化，并重跑三版本测试。
